...
Die erste Option erklärt sich im Grunde von selbst. Technisch völlig problemfrei ist es, einfach nur eine einzige Instanz von Red Hat GitOps zu verwenden. Die notwendige Trennung der Berechtigungen erfolgt, wie immer, über die entsprechende RBAC Konfiguration in Red Hat GitOps, so dass die Developerteams nur Berechtigungen in ihrem Argo-Applikationsprojekt erhalten. Mit den Berechtigungen muss man hier natürlich noch einmal besonders aufpassen, damit niemand unberechtigtes die Rechte zur Clusteradministration erhält.
